...
class Recommendation(Base):
    ...
    title: Mapped[str] = mapped_column(
        String(300), nullable=False, index=True, comment="Recommendation title"
    )

    description: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Detailed description of the recommendation"
    )

    recommendation_type: Mapped[RecommendationType] = mapped_column(
        SQLEnum(RecommendationType), nullable=False, index=True, comment="Type of recommendation"
    )
    ...
    priority: Mapped[Priority] = mapped_column(
        SQLEnum(Priority), nullable=False, index=True, comment="Priority level for implementation"
    )

    # Classification and categorization
    category: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, index=True, comment="Recommendation category"
    )

    subcategory: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, index=True, comment="Recommendation subcategory"
    )
    ...
    control_id: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="Control identifier from security framework"
    )

    tags: Mapped[Optional[Dict[str, str]]] = mapped_column(
        JSON, nullable=True, comment="Tags for categorization and filtering"
    )

    # Implementation guidance
    implementation_steps: Mapped[Optional[List[Dict[str, Any]]]] = mapped_column(
        JSON, nullable=True, comment="Step-by-step implementation instructions"
    )

    remediation_code: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Code snippets or commands for remediation"
    )

    rollback_procedure: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Procedure to rollback changes if needed"
    )

    testing_instructions: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Instructions to test the implementation"
    )

    # Impact assessment
    impact_assessment: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Assessment of potential impact"
    )

    risk_reduction: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Estimated risk reduction percentage"
    )

    implementation_effort: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="Estimated implementation effort (low/medium/high)"
    )

    implementation_cost: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="Estimated implementation cost (low/medium/high)"
    )
    ...
    assigned_to: Mapped[Optional[str]] = mapped_column(
        String(200), nullable=True, index=True, comment="Person or team assigned for implementation"
    )

    due_date: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True, comment="Due date for implementation"
    )
    ...
    rejection_reason: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Reason for rejecting the recommendation"
    )

    deferral_reason: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Reason for deferring the recommendation"
    )
    ...
    external_reference: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True, comment="External reference or documentation link"
    )

    cve_id: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True, index=True, comment="Related CVE identifier if applicable"
    )

    # Business context
    business_impact: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Business impact description"
    )

    business_justification: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Business justification for implementation"
    )

    # Monitoring and verification
    verification_method: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Method to verify implementation"
    )

    monitoring_required: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False, comment="Whether ongoing monitoring is required"
    )

    success_criteria: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True, comment="Success criteria for implementation"
    )
    ...
    policy: Mapped[Optional["Policy"]] = relationship("Policy", back_populates="recommendations")
